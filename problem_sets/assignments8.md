...
mass_loop <- numeric(length(dinosaur_lengths))

for (i in 1:length(dinosaur_lengths)) {
  mass_loop[i] <- a_values[i] * (dinosaur_lengths[i] ^ b_values[i])
}
mass_loop
```
...
All the parts are the same except the file name in years’ part.
...
for (year in start:end){
  path <- str_c(
    'https://raw.githubusercontent.com/nt246/NTRES-6100-data-science/master/datasets/buoydata/44013_',year,'.csv'
  )
  print(path)
}
```
...
df1<-df_combined |> select(YY,MM,WVHT,WTMP) |> 
    rename(
    year = YY,
    month = MM,
    wave_heights=WVHT,
  ...
  group_by(year, month) |>
    summarize(
      wav_height_avg=mean(wave_heights, na.rm = TRUE),
      temp_avg=mean(temperatures, na.rm = TRUE)
    ) |> 
  mutate(
    year_month = str_c(year, "-", month, "-01") |> 
      lubridate::ymd()
  ) 
  

df1 |> ggplot(aes(x = year_month, y = wav_height_avg)) +
geom_line() +
  geom_point()
```

![](assignments8_files/figure-commonmark/unnamed-chunk-8-1.png)

``` r
df1 |> 
  ggplot(aes(x = year_month, y = temp_avg)) +
  geom_line()+
  geom_point()
```

![](assignments8_files/figure-commonmark/unnamed-chunk-8-2.png)
